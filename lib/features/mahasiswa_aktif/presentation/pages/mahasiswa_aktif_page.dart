        ...
        children: [
          ...
          Expanded(
            child: mahasiswaState.when(
              ...
              data: (mahasiswaList) => _MahasiswaAktifListWithSave(
            ...
            children: [
              ...
              savedUsers.maybeWhen(
                data: (users) => users.isNotEmpty
                    ? TextButton.icon(
                        onPressed: () async {
                          await ref.read(mahasiswaAktifNotifierProvider.notifier).clearSavedUsers();
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return ListTile(
                      ...
                      trailing: IconButton(
                        ...
                        onPressed: () async {
                          await ref.read(mahasiswaAktifNotifierProvider.notifier).removeSavedUser(user['user_id'] ?? '');
                          ref.invalidate(savedUsersProvider);
                          if (context.mounted) {
                            ScaffoldMessenger.of(context).showSnackBar(
...
class _MahasiswaAktifListWithSave extends ConsumerWidget {
  ...
  const _MahasiswaAktifListWithSave({required this.mahasiswaList, required this.onRefresh});
        ...
        itemBuilder: (context, index) {
          ...
          return Card(
            ...
            child: ListTile(
              ...
              title: Text(mahasiswa.title, maxLines: 1, overflow: TextOverflow.ellipsis),
              subtitle: Text(
                mahasiswa.body,
                maxLines: 2,
              ...
              trailing: IconButton(
                ...
                onPressed: () async {
                  await ref.read(mahasiswaAktifNotifierProvider.notifier).saveSelectedMahasiswa(mahasiswa);
                  ref.invalidate(savedUsersProvider);
                  if (context.mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(content: Text('${mahasiswa.title} berhasil disimpan ke local storage')),
                    );
                  }
                },
              ),
