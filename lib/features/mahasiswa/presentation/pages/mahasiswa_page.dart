...
import 'package:aplikasi_mobile/features/mahasiswa/presentation/widgets/mahasiswa_widget.dart'; // Sesuaikan jika ada folder widgets
...
class MahasiswaPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final mahasiswaState = ref.watch(mahasiswaNotifierProvider); // CHANGED [1]
    final savedUsers = ref.watch(savedUsersProvider); // CHANGED [1]

    return Scaffold(
      appBar: AppBar(
        title: const Text('Data Mahasiswa'), // CHANGED [1]
        elevation: 0,
        actions: [
          IconButton(
        ...
        children: [
          ...
          Expanded(
            child: mahasiswaState.when(
              loading: () => const Center(child: CircularProgressIndicator()), // Atau Custom LoadingWidget() [1]
              error: (error, stack) => Center(
                child: Column(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    Text('Gagal memuat data mahasiswa: ${error.toString()}'), // CHANGED [1]
                    ElevatedButton(
              ...
              data: (mahasiswaList) => _MahasiswaListWithSave( // CHANGED [1]
            ...
            children: [
              ...
              savedUsers.maybeWhen(
                data: (users) => users.isNotEmpty
                    ? TextButton.icon(
                        onPressed: () async {
                          await ref.read(mahasiswaNotifierProvider.notifier).clearSavedUsers(); // CHANGED [1]
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return ListTile(
                      ...
                      trailing: IconButton(
                        ...
                        onPressed: () async {
                          await ref.read(mahasiswaNotifierProvider.notifier).removeSavedUser(user['user_id'] ?? ''); // CHANGED [1]
                          ref.invalidate(savedUsersProvider);
                          if (context.mounted) {
                            ScaffoldMessenger.of(context).showSnackBar(
...
class _MahasiswaListWithSave extends ConsumerWidget { // CHANGED [1]
  final List<MahasiswaModel> mahasiswaList; // CHANGED [1]
  final VoidCallback onRefresh;

  const _MahasiswaListWithSave({required this.mahasiswaList, required this.onRefresh}); // CHANGED [1]
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return RefreshIndicator(
      ...
      child: ListView.builder(
        itemCount: mahasiswaList.length, // CHANGED [1]
        padding: const EdgeInsets.fromLTRB(12, 0, 12, 12),
        itemBuilder: (context, index) {
          final mahasiswa = mahasiswaList[index]; // CHANGED [1]
          return Card(
            margin: const EdgeInsets.symmetric(vertical: 6),
            child: ListTile(
              leading: CircleAvatar(child: Text('${mahasiswa.id}')), // CHANGED [1]
              title: Text(mahasiswa.name), // CHANGED [1]
              subtitle: Text(
                '${mahasiswa.email}\n${mahasiswa.body}', // CHANGED [1]
              ),
              isThreeLine: true,
              trailing: IconButton(
                ...
                onPressed: () async {
                  await ref.read(mahasiswaNotifierProvider.notifier).saveSelectedMahasiswa(mahasiswa); // CHANGED [1]
                  ref.invalidate(savedUsersProvider);
                  if (context.mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(content: Text('${mahasiswa.name} berhasil disimpan ke local storage')), // CHANGED [1]
                    );
                  }
                },
              ),
